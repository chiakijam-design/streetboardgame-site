...
- 認証: 管理トークンと認証アプリのTOTPによる二要素認証。通常の管理セッションは15分。「このパソコンでは30日間、再認証を省略する」を選んだ信頼済み端末は30日
- 管理トークン、管理セッション、TOTPはURLへ付けない。管理トークンとTOTPは入力中のメモリだけで扱い、認証成功時に入力欄から消す。通常セッションは`sessionStorage`、信頼済み端末セッションは`localStorage`に保存する。共有端末では信頼設定を外し、作業後に「保存した認証情報を消す」を実行する。
...
通常の予約管理は企画保存時に発行したスタッフ用URLから行う。スタッフ用URLを紛失した場合など、運営が代理対応するときは二要素認証後の運営コンソールから対象予約の「予約キャンセル」を使う。

1. 変更時は新しい日時を選び、「変更先の空きを確認」を押す。
2. 空きがある場合だけ確定する。確定に失敗した場合、現在の予約枠は保持される。
3. キャンセル時は対象企画と日時を再確認して実行する。スタッフ画面・運営コンソールのどちらから実行しても予約枠は直ちに解放され、元に戻せない。
4. ライブ開始後は日時変更と予約キャンセルを行えない。障害時は運営コンソールの「強制終了」、URL漏えい時は同画面のURL再発行を使う。
