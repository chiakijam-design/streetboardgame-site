...
function renderMetrics() {
  ...
  document.getElementById('metrics').innerHTML = [
    ...
    metric('本番接続', [infra.d1Configured && 'ゲームD1', infra.purchaseD1Configured && '購入D1', infra.durableObjectsConfigured && 'DO', infra.privateR2Configured && '非公開R2', infra.imagesBindingConfigured && 'Images', infra.alertWebhookConfigured && '通知Webhook', infra.stripeCheckoutConfigured && 'Stripe Checkout', infra.stripeWebhookConfigured && 'Stripe Webhook'].filter(Boolean).join(' / ') || '未設定', '現在のWorker実行環境'),
  ].join('');
}
...
function renderEvents() {
  ...
  const recentThreshold = Number(overview.generatedAt || Date.now()) - 15 * 60 * 1000;
  document.getElementById('events').innerHTML = events.length ? events.map((item) => {
    const isRecent = Number(item.created_at || 0) >= recentThreshold;
    return `<article class="card"><strong><span class="pill ${escapeAttr(item.severity)}">${escapeHtml(item.severity)}</span><span class="pill ${isRecent ? 'warning' : ''}">${isRecent ? '直近15分' : '過去ログ'}</span>${escapeHtml(item.event_type)}</strong><div class="meta">${formatDate(item.created_at)} / ${escapeHtml(item.category)} ${item.code ? `/ room ${escapeHtml(item.code)}` : ''}<br>${escapeHtml(item.message)}</div>${item.acknowledged_at ? `<span class="pill info">確認済み ${formatDate(item.acknowledged_at)}</span>` : `<div class="actions"><button class="button secondary" data-ack="${escapeAttr(item.event_id)}">確認済みにする</button></div>`}</article>`;
  }).join('') : empty('監視イベントはありません。');
...
async function refundCheckout(orderId, execute) {
  if (!confirm(execute ? 'Stripe APIでこの注文を全額返金しますか？元の支払方法へ返金されます。' : '購入権限を即時停止し、配信者分配を保留して返金待ちにしますか？')) return;
