...
export function inferReviewCategory(card = {}) {
  ...
  if (/旅行|行きたい|場所|都道府県|地域|遊び|休日|パーティー|イベント/.test(source)) return 'outing';
  if (/家|部屋|暮らし|寝る前|お風呂|朝|支度|持ち物|常備/.test(source)) return 'lifestyle';
  if (/性格|価値観|自分|基準|信じ|人生|言葉|属性/.test(source)) return 'personality';
...
function topCategory(bucket, fallback) {
  return Object.entries(bucket).sort((left, right) => right[1] - left[1])[0]?.[0] || fallback;
}
...
export function createReviewContext(answers = [], cards = []) {
  ...
  const questionSeed = safeCards.reduce((sum, card, index) => {
    ...
    const textSeed = Array.from(text).reduce((value, character) => value + character.charCodeAt(0), 0);
...
export function createReviewTemplateTools(values, context) {
  ...
  const pickVariant = (list, offset = 0) => {
    ...
    const seed = context.questionSeed + (context.score * 13) + (context.hit.length * 5)
      + (context.miss.length * 7) + offset;
