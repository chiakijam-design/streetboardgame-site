...
test('結果画面から高画質画像と応援金のStripe Checkoutを開始できる', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '1,000円で生成・ダウンロードを申し込む' })).toBeVisible();
  ...
  expect(checkoutBodies[0].body).toMatchObject({ productType: 'support', amount: 500, viewerName: '視聴者A' });
  ...
  expect(checkoutBodies[1].body).toMatchObject({ productType: 'result_image', viewerName: '視聴者A' });
...
test('招待コード発行手順を確認しLIVE問い合わせを送信できる', async ({ page }) => {
  ...
  await expect(page.locator('#liveRoot .shell > section.panel > h2')).toHaveText([
    'YouTubeチャンネルから問題を作る',
    'ルームに参加する',
    '購入済み画像を再ダウンロード',
