...
test('トップの家族ボタン直下からLIVEを開始し、紹介カードから説明ページへ移動できる', async ({ page }) => {
  ...
  const playLink = page.getByRole('link', { name: 'LIVEゲームを作って遊ぶ', exact: true });
  ...
  await expect(page.getByRole('heading', { name: '私のこと、ちゃんとわかってるよね？LIVE' })).toBeVisible();
  await expect(page.getByRole('heading', { name: '自分で作った問題を、みんなでライブ投票' })).toBeVisible();
  await expect(page.getByRole('link', { name: 'LIVEゲームを作って遊ぶ' })).toHaveAttribute('href', '/live');

  await page.goto('/');
  await page.getByRole('link', { name: 'LIVEゲームを作って遊ぶ', exact: true }).click();
