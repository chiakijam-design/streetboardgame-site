...
test('YouTubeの本人回答モードだけ30問を生成し、1問以上を選んで共通編集へ進む', async ({ page }) => {
  ...
  await expect(page.getByText(/本人の答えはここでは設定しません/).first()).toBeVisible();
...
test('YouTube候補から作った問題を投票し、秘密回答を締切前に公開しない', async ({ browser, page, request }) => {
  await page.goto('/live');
  await page.route('**/api/live/youtube-candidates', async (route) => {
    const questions = Array.from({ length: 30 }, (_, index) => ({
      id: `flow-${index}`,
  ...
  await expect(participant.getByText(/本人が回答中/)).toBeVisible();
  await expect(participant.locator('[data-vote-index]')).toHaveCount(0);
  const publicAnswering = await request.get(`/api/live/games/${roomCode}`);
  expect(JSON.stringify(await publicAnswering.json())).not.toContain('lockedIndex');
  await page.locator('[data-host-answer-index="0"]').click();
  await page.locator('#confirmHostAnswer').click();
  ...
  await expect(participant.getByText('1 / 1問正解')).toBeVisible();
  await expect(participant.locator('.result-card')).toHaveCount(1);
...
test('トップの家族ボタン直下からLIVEを開始し、紹介カードから説明ページへ移動できる', async ({ page }) => {
  ...
  const playLink = page.getByRole('link', { name: 'YouTuber向けLIVEを作って遊ぶ', exact: true });
  ...
  const guideLink = page.getByRole('link', { name: /私のこと、ちゃんとわかってるよね？LIVE/ });
  await expect(guideLink).toHaveAttribute('href', '/live-guide');
  await guideLink.click();
  await expect(page).toHaveURL('/live-guide');
  await expect(page.getByRole('heading', { name: 'YouTuber向け 私のこと、ちゃんとわかってるよね？LIVE' })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'YouTubeチャンネルから作る、視聴者参加型LIVE' })).toBeVisible();
  await expect(page.getByRole('link', { name: 'YouTuber向けLIVEを作って遊ぶ' })).toHaveAttribute('href', '/live');

  await page.goto('/');
  await page.getByRole('link', { name: 'YouTuber向けLIVEを作って遊ぶ', exact: true }).click();
