...
test('LIVE運営コンソールで監視・予約・購入対応を確認できる', async ({ page }) => {
  ...
  await page.route('**/api/live/admin/games/123456/cancel', async (route) => {
    reservationCancels.push(route.request().postDataJSON());
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ code: '123456', game: { phase: 'cancelled' } }) });
  });
  ...
  await expect(reservationCard.getByRole('button', { name: '予約キャンセル' })).toBeVisible();
  await expect(reservationCard.getByRole('button', { name: '強制終了' })).toHaveCount(0);
  await expect(activeCard.getByRole('button', { name: '強制終了' })).toBeVisible();
  await expect(activeCard.getByRole('button', { name: '予約キャンセル' })).toHaveCount(0);
  page.once('dialog', (dialog) => dialog.accept());
  await reservationCard.getByRole('button', { name: '予約キャンセル' }).click();
  await expect.poll(() => reservationCancels.length).toBe(1);
  expect(reservationCancels[0]).toEqual({});
  page.once('dialog', (dialog) => dialog.accept());
  await reservationCard.getByRole('button', { name: '本人URL失効・再発行' }).click();
  await expect.poll(() => linkRotations.length).toBe(1);
  expect(linkRotations[0]).toEqual({ host: false, subject: true });
  await expect(page.locator('#privateLinkOutput')).toContainText('新しい本人URL（今回だけ表示）');
