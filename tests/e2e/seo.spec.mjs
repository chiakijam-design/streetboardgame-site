...
test('内部リンクを実URLで辿れ、主要ランドマークが存在する', async ({ page }, testInfo) => {
  ...
  for (const path of ['/remote', '/live', '/live-guide', '/love', '/friends', '/family', '/about', '/product', '/terms', '/legal']) {
...
test('作成・運営画面と秘密情報付きURLを検索対象から除外する', async ({ page, request }) => {
  const liveResponse = await page.goto('/live');
  expect(liveResponse?.headers()['x-robots-tag']).toContain('noindex');
...
test('法務ページに販売条件・分配条件・非提携表示があり相互に移動できる', async ({ page }) => {
  ...
  await expect(page.getByText(/対象商品の売上総額（税込）の70%/)).toBeVisible();
...
test('sitemap・robots・末尾スラッシュの正規化が一致する', async ({ request }, testInfo) => {
  ...
  for (const path of ['/love', '/remote', '/friends', '/family', '/live-guide', '/about', '/product', '/terms', '/privacy', '/legal', '/creator-terms', '/refund-policy', '/content-guidelines', '/minor-policy']) {
