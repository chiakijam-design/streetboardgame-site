...
test('サイトマップ掲載URLは200・自己canonical・index可能で統一する', async ({ request }, testInfo) => {
  ...
  for (const updatedUrl of [
    ...
    const expectedDate = [`${ORIGIN}/`, `${ORIGIN}/challenge`, `${ORIGIN}/en/challenge`].includes(updatedUrl) ? '2026-09-16' : [
      `${ORIGIN}/privacy`,
      `${ORIGIN}/en/privacy`,
...
test('表示フォントはNoto Sans JPに統一する', async ({ request }, testInfo) => {
  test.skip(testInfo.project.name === 'mobile-chrome', 'HTMLとCSSは画面幅に依存しないためPCで1回検証');
  const fontStylesheet = 'family=Noto+Sans+JP';
  ...
  expect(topScript).toContain('Noto Sans JP');
