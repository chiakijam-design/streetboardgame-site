...
const candidateDataUrl = new URL('../../data/question-candidates-20260727-300.json', import.meta.url);
...
test('追加候補300問は5択・出典・安全性の条件を満たす', async () => {
  ...
  assert.deepEqual(
    ...
    Array.from({ length: 300 }, (_, index) => `HLD${String(index + 201).padStart(3, '0')}`),
  );
  assert.equal(new Set(candidates.map((candidate) => candidate.title)).size, 300);
  assert.equal(new Set(candidates.map((candidate) => candidate.sourceRef)).size, 300);
...
test('追加候補300問は既存問題・過去200候補と強く重複しない', async () => {
  globalThis.window = {};
...
test('追加候補300問はD1へ保留・通常版OFF・LIVE版OFFで入る', async () => {
  const sqlite = new DatabaseSync(':memory:');
  sqlite.exec(await readFile(new URL('../../migrations/0012_question_catalog_moderation.sql', import.meta.url), 'utf8'));
  sqlite.exec(await readFile(new URL('../../migrations/0019_add_held_question_candidates.sql', import.meta.url), 'utf8'));
  sqlite.exec(await readFile(new URL('../../migrations/0021_add_more_held_question_candidates.sql', import.meta.url), 'utf8'));
  sqlite.exec(await readFile(new URL('../../migrations/0022_add_300_held_question_candidates.sql', import.meta.url), 'utf8'));

  assert.equal(sqlite.prepare("SELECT COUNT(*) AS count FROM question_catalog WHERE source_kind = 'candidate'").get().count, 500);
  assert.equal(sqlite.prepare(`
    SELECT COUNT(*) AS count
    FROM question_catalog
    WHERE question_id BETWEEN 'HLD201' AND 'HLD500'
      AND status = 'held'
      AND use_challenge = 0
      AND use_live = 0
  `).get().count, 300);
