...
test('応援金Checkoutは選択金額と応援商品名をStripeへ送り、画像再取得情報を付けない', async () => {
  let captured;
  const env = stripeEnv(async (url, options) => {
    captured = { url, options, params: new URLSearchParams(options.body) };
    return Response.json({ id: 'cs_test_support01', url: 'https://checkout.stripe.com/c/pay/support', expires_at: 1_800_001_800 });
  });
  const session = await createLiveCheckoutSession(env, {
    requestUrl: 'https://www.streetboardgame.com/api/live/games/123456/checkout',
...
test('応援金Webhookは画像サービスなしで注文を確定し、画像購入権限を発行しない', async () => {
  const purchaseDb = new PurchaseDb();
  purchaseDb.order = {
  ...
  const payload = JSON.stringify(event);
  const signature = await stripeSignature(env.STRIPE_WEBHOOK_SECRET, payload);
  const response = await handleLiveApi(new Request('https://www.streetboardgame.com/api/live/stripe/webhook', {
    method: 'POST', headers: { 'stripe-signature': signature }, body: payload,
  }), env, '/api/live/stripe/webhook');

  assert.equal(response.status, 200);
  assert.equal(purchaseDb.order.status, 'paid');
  assert.equal(purchaseDb.order.stripe_payment_intent_id, 'pi_support_webhook01');
  assert.equal(purchaseDb.order.purchase_id, null);
  assert.equal(purchaseDb.entitlements.length, 0);
});
