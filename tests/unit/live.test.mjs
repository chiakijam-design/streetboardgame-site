...
test('非アンケート問題は非公開回答が必要で、選択肢は2〜4個に正規化する', () => {
...
test('YouTubeの一般的なチャンネルURLを正規化し、15問ずつ合計30問を作る', () => {
  ...
  const questions = generateYouTubeQuestions({ channelName: 'サンプル', videoTitles: ['動画Aです', '動画Bです', '動画Cです', '動画Dです'] });
  assert.equal(questions.length, 30);
  assert.equal(questions.filter(({ type }) => type === 'guess-person').length, 15);
  assert.equal(questions.filter(({ type }) => type === 'guess-majority').length, 15);
  assert.equal(recommendYouTubeCandidates(questions).filter(({ selected }) => selected).length, 10);
});
