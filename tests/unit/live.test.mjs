...
import { fetchOwnedYouTubeCaptionSources, fetchYouTubeDataProfile, normalizeCaptionText } from '../../src/live/youtube.js';
...
test('審査待ちのYouTuber画像は公開利用可能として返さない', () => {
  const base = {
    version: 5, title: '画像審査', subjectName: '本人', phase: 'lobby', currentQuestionIndex: 0,
    questions: [{ id: 'q-1', type: 'guess-person', text: '問題', options: ['A', 'B'], lockedIndex: null }],
    participants: [], votes: {}, results: [], creatorImage: { previewKey: 'private', moderationStatus: 'pending' },
  };
  assert.equal(publicLiveGame(base).hasCreatorImage, false);
  assert.equal(publicLiveGame(base, { host: true }).creatorImageModerationStatus, 'pending');
  assert.equal(publicLiveGame({ ...base, creatorImage: { previewKey: 'private', moderationStatus: 'approved' } }).hasCreatorImage, true);
});
...
test('参加者だけに各問題の自分の回答と正誤を返す', () => {
  const game = {
    ...
    creatorImageDataUrl: 'data:image/webp;base64,QUJD',
    creatorImage: {
      originalKey: 'live/123456/creator/private/original',
      previewKey: 'live/123456/creator/private/preview.webp',
      paidKey: 'live/123456/creator/private/paid.webp',
    },
    channelName: '本人チャンネル', scheduledAt: Date.now() + 1000,
    questions: [{ id: 'q-1', type: 'guess-person', text: '本人の答え', options: ['A', 'B'], lockedIndex: 1 }],
    participants: [{ id: 'p-1', token: 'secret-participant', name: '参加者' }],
    votes: { 'q-1': { 'p-1': 1 } },
  ...
  assert.equal('creatorImageDataUrl' in participantResult, false);
  assert.equal(participantResult.hasCreatorImage, true);
  assert.equal(JSON.stringify(participantResult).includes('originalKey'), false);
  assert.equal(JSON.stringify(participantResult).includes('previewKey'), false);
});
...
test('YouTube情報は公式Data API v3だけで動画から投稿元と最近の動画を取得する', async () => {
  const requests = [];
  const apiFetch = async (request) => {
    ...
    if (url.pathname.endsWith('/videos') && url.searchParams.get('id') === 'HTRGCp7sDpl') {
      return Response.json({ items: [{
        id: 'HTRGCp7sDpl',
        snippet: { channelId: 'UC1234567890_sample', title: '入力動画', description: '入力動画の説明', tags: ['内輪'] },
      }] });
    }
    if (url.pathname.endsWith('/channels')) {
      return Response.json({ items: [{
        id: 'UC1234567890_sample',
        snippet: { title: '公式チャンネル', description: 'チャンネル説明', customUrl: '@official' },
        contentDetails: { relatedPlaylists: { uploads: 'UU1234567890_sample' } },
      }] });
    }
    if (url.pathname.endsWith('/playlistItems')) {
      return Response.json({ items: [
        { contentDetails: { videoId: 'recentVideo01' } },
        { contentDetails: { videoId: 'recentVideo02' } },
      ] });
    }
    if (url.pathname.endsWith('/videos')) {
      return Response.json({ items: [
        { id: 'recentVideo01', snippet: { title: '最近の企画A', description: '裏話A', tags: ['企画A'] } },
        { id: 'recentVideo02', snippet: { title: '最近の企画B', description: '裏話B', tags: ['企画B'] } },
      ] });
    }
    return Response.json({}, { status: 404 });
  };
  const profile = await fetchYouTubeDataProfile('https://youtu.be/HTRGCp7sDpl', {
    YOUTUBE_API_KEY: 'test-api-key', YOUTUBE_API_FETCH: apiFetch,
  });
  assert.equal(profile.source, 'youtube-data-api-v3');
  assert.equal(profile.channelId, 'UC1234567890_sample');
  assert.equal(profile.channelName, '公式チャンネル');
  assert.deepEqual(profile.videoTitles, ['入力動画', '最近の企画A', '最近の企画B']);
  assert.equal(requests.some((url) => url.pathname.endsWith('/playlistItems')), true);
  assert.equal(requests.every((url) => !url.hostname.endsWith('youtube.com')), true);
});

test('YouTube公式APIキー未設定時はHTML取得へフォールバックせず停止する', async () => {
  await assert.rejects(
    fetchYouTubeDataProfile('https://www.youtube.com/@sample', {}),
    (error) => error.message === 'youtube-api-not-configured' && error.status === 503,
  );
});
...
test('YouTuber本人のOAuthで公開動画字幕を取得し、VTTを問題生成用本文へ整形する', async () => {
  const requests = [];
  const apiFetch = async (request, options = {}) => {
    ...
    if (url.pathname.endsWith('/captions')) {
      return Response.json({ items: [{
        id: 'caption-track-ja',
        snippet: { language: 'ja', trackKind: 'ASR', status: 'serving', isDraft: false },
      }] });
    }
    if (url.pathname.endsWith('/captions/caption-track-ja')) {
  ...
  const sources = await fetchOwnedYouTubeCaptionSources({
    videoSummaries: [{ videoId: 'recentVideo01', title: '夏合宿ドッキリ' }],
  }, 'oauth-access-token', apiFetch);
  assert.equal(sources.length, 1);
  assert.equal(sources[0].title, '夏合宿ドッキリ');
  assert.match(sources[0].transcript, /秘密の罰ゲーム/);
  assert.equal(sources[0].transcript.includes('00:00'), false);
  assert.equal(requests.every(({ authorization }) => authorization === 'Bearer oauth-access-token'), true);
  assert.equal(normalizeCaptionText('WEBVTT\n\n00:00:00.000 --> 00:00:01.000\n同じ発言\n同じ発言'), '同じ発言');
});

test('本人許可済み字幕の実際の発言を30問の内輪向け候補へ反映する', () => {
  const transcript = [
    '幼なじみ四人で夏合宿に来ました。',
    '秘密の罰ゲームは本人だけが知っています。',
    '朝ごはんのカレーを作ったのはみゆゆです。',
    ...
    '夜の花火で昔の写真の話になりました。',
    '帰りの車では次の旅行先を相談しました。',
    '撮影前にドッキリの合図を三回確認しました。',
  ].join('\n');
  const profile = {
    channelName: 'わたちゃんず',
    videoTitles: ['夏合宿ドッキリ', '幼なじみ王決定戦'],
    contentSources: [{ videoId: 'summerTrip01', title: '夏合宿ドッキリ', transcript }],
  };
  const moments = extractYouTubeContentMoments(profile);
  assert.equal(moments.length >= 5, true);
  const questions = generateYouTubeQuestions(profile, 0, 'guess-person');
  assert.equal(questions.length, 30);
  assert.equal(questions.every(({ options }) => options.length === 5), true);
  assert.equal(new Set(questions.map(({ text }) => text)).size, 30);
  assert.equal(questions.slice(0, 20).some(({ text, options }) => (
    text.includes('秘密の罰ゲーム') || options.some((option) => option.includes('秘密の罰ゲーム'))
  )), true);
});
