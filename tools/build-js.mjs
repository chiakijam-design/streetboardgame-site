...
const prerenderBundle = await build({
  stdin: {
    contents: "import './prototype_character.jsx'; export { TopPage, ChallengeGuide, AboutPage, ProductPage } from './prototype_app.jsx';",
...
const renderContext = vm.createContext({ React, URLSearchParams, location: { search: '' } });
...
// Reuse the real components, not a second SEO-only version of their content.
const infoMarkup = Object.fromEntries(Object.entries({
  '/challenge-guide': 'ChallengeGuide',
  '/about': 'AboutPage',
  '/product': 'ProductPage',
}).map(([path, component]) => [path, renderToString(React.createElement(renderContext.HomePrerender[component]))]));
await mkdir('src/generated', { recursive: true });
await writeFile('src/generated/info-pages.js', `// Generated by tools/build-js.mjs. Do not edit.\nexport const infoPageMarkup = ${JSON.stringify(infoMarkup, null, 2)};\n`);
